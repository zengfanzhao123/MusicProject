...
<script>
import {login,user} from '@/http/api'
export default {
    ...
    methods:{
        ...
        setLog(){
            ...
            user.getUserPlayerlist(data.id).then(res=>{
                //id name coverImgUrl
                res.data.playlist.forEach(e => {
                    e.coverImgUrl = e.coverImgUrl + '?' + 'param=50y50'
                })
                this.$store.state.playlist = res.data.playlist
            })
        }
    },
    mounted(){
